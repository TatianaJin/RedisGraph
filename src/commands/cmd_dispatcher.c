  ...
  *batch_size = 16;
    ...
    if (!strcasecmp(arg, "batch")) {
      ...
      if (i < argc - 1) {
        ...
        err = RedisModule_StringToLongLong(argv[i], &v);
        *batch_size = v;
        printf("set batch_size=%ld\n", *batch_size);
      }
      if (err != REDISMODULE_OK || *batch_size < 0) {
        asprintf(errmsg, "Failed to parse batch size value");
        return err;
      }
    }
	}
...
static GRAPH_Commands determine_command(const char *cmd_name) {
	...
	if(strcasecmp(cmd_name, "graph.INFO") == 0) return CMD_INFO;
